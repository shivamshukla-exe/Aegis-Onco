...
import {
  ...
  Brain, BarChart3, FlaskConical, Wifi,
} from 'lucide-react';
import {
...
const TumorVisualization = lazy(() => import('../components/TumorVisualization'));
...
// ─────────────────────────────────────────────────────────────────────────────
// Seeded variation gauge — RadialBarChart
// ─────────────────────────────────────────────────────────────────────────────
function VariationGauge({ value }: { value: number }) {
  const pct = Math.min(value / 1.5, 1) * 100;
  const warning = value > 0.8;
  const data = [{ name: 'σ', value: pct, fill: warning ? '#f43f5e' : value > 0.5 ? '#f59e0b' : '#10b981' }];
  return (
    <div className="relative h-[120px] w-full">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <motion.span key={value.toFixed(2)} initial={{ scale: 1.15, opacity: 0.5 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.25 }}
          className={`font-mono-data text-[22px] font-extrabold ${warning ? 'text-rose-500' : value > 0.5 ? 'text-amber-600' : 'text-emerald-600'}`}>
          {value.toFixed(2)}
        </motion.span>
        <span className="text-[8px] text-slate-400 uppercase tracking-wider font-mono-data">σ Seeded Variation</span>
      </div>
    </div>
  );
}
...
export default function CommandCenter() {
  ...
  const {
    baseSurrogateScore: baseSurrogateScore, meanSurrogateScore: surrogateScore, variationStd: variationStd, samples, curve,
    medianDay, s365, s730, s1095, s2000, variationWarning,
  } = engine;
  ...
  const statusMessage = useMemo(() => {
    let msg = `Research demo calculation: Complete\n`;
    msg += `Hand-authored surrogate score: ${surrogateScore.toFixed(4)}\n`;
    msg += `Seeded simulation variation (σ): ${variationStd.toFixed(4)}\n`;
    if (variationStd > 0.8) {
      msg += `WARNING: Seeded variation exceeds the UI display threshold. This is not a confidence or clinical quality assessment.`;
    }
    return msg;
  }, [surrogateScore, variationStd]);

  // Synthetic research-demo topology, not connected institutions.
  const topoNodes = FEDERATED_NODES.slice(0, 3);

  // ── Synthetic demo metrics ──
  const quickMetrics: { label: string; val: string; accent: Accent }[] = [
    { label: 'Base Surrogate Score', val: baseSurrogateScore.toFixed(4), accent: 'violet' },
    { label: 'Mean Seeded Score', val: surrogateScore.toFixed(4), accent: 'blue' },
    { label: 'Seeded Variation σ', val: variationStd.toFixed(4), accent: variationWarning ? 'rose' : 'emerald' },
    { label: 'Demo Median Day', val: medianDay !== null ? `${medianDay} days` : '> 2000 days', accent: 'amber' },
  ];

  return (
    <div className="relative">
      {/* ── Page Header ── */}
      <PageHeader icon={Activity} title="Command Center" subtitle="Synthetic Research Surrogate · Not for Clinical Use" accent="violet">
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          <motion.div whileHover={{ scale: 1.04 }} className="glass-card-emerald rounded-xl px-3.5 py-2 flex items-center gap-2 glow-emerald">
            <FlaskConical className="text-emerald-600" size={14} />
            <span className="text-[9px] uppercase tracking-wider text-emerald-700 font-bold">Synthetic Research Demo</span>
          </motion.div>
          <div className="glass-card-violet rounded-xl px-3.5 py-2 flex items-center gap-2 glow-violet">
            ...
            <span className="text-[9px] uppercase tracking-wider text-violet-700 font-bold">Demo Telemetry</span>
      ...
      <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: 'easeOut' }}
        ...
        <div className="flex items-center justify-between px-5 pt-4 pb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-[13px] font-bold text-slate-800">3D Synthetic Morphology Illustration</h3>
              <p className="text-[9px] text-slate-400 font-mono-data uppercase tracking-[0.18em] mt-0.5">Stylized Synthetic View · Not Imaging or Anatomy · Drag to Rotate</p>
            </div>
          </div>
        ...
        <div className="h-[400px] w-full">
          <Suspense fallback={<div className="flex h-full items-center justify-center font-mono-data text-[11px] text-slate-500">Loading optional 3D demo…</div>}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-5">
        ...
        <div className="space-y-5">
          {/* Module 1: synthetic clinical inputs */}
          ...
          {/* Module 4: non-causal scenario offset */}
                ...
                <AnimatePresence mode="wait">
                  {isRunning ? (
                    <motion.span key="running" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                      ...
                      Computing deterministic surrogate variation...
                    </motion.span>
                  ) : (
                    <motion.span key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                      className="flex items-center justify-center gap-2">
                      <Zap size={16} />
                      Execute Deterministic Demo Run
                    </motion.span>
                  )}
                </AnimatePresence>
                {/* Shimmer overlay */}
        ...
        <div className="space-y-5">
          ...
          <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: 'easeOut' }}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-[15px] font-bold text-slate-800">Synthetic Surrogate Curve</h2>
                  <p className="text-[10px] text-slate-400 font-mono-data uppercase tracking-[0.15em] mt-0.5">
                    Hand-authored baseline · 20 seeded perturbations · Scenario: {intervention}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-4 text-[10px] font-mono-data">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-slate-500">Simulation range</span>
                </div>
                {medianDay !== null && (
                  <div className="flex items-center gap-1.5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}
              ...
              <div className="flex items-center gap-2.5 mb-4">
                ...
                <div>
                  <h3 className="text-[12px] font-bold text-slate-800">Seeded Surrogate Variation</h3>
                  <p className="text-[9px] text-slate-400 font-mono-data uppercase tracking-wider">20 Deterministic Perturbations · Not MC Dropout</p>
                </div>
              </div>
              <MCHistogram samples={samples} mean={surrogateScore} />
              <div className="flex justify-between mt-2 text-[10px] font-mono-data">
                <span className="text-slate-400">μ = <span className="text-violet-600 font-bold">{surrogateScore.toFixed(4)}</span></span>
                <span className="text-slate-400">σ = <span className={variationWarning ? 'text-rose-500 font-bold' : 'text-emerald-600 font-bold'}>{variationStd.toFixed(4)}</span></span>
              </div>
            </motion.section>

            {/* Seeded variation gauge */}
            <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.16 }}
              whileHover={{ y: -2 }} className={`rounded-3xl p-5 ${variationWarning ? 'glass-card-rose glow-rose' : 'glass-card-emerald'}`}>
              <div className="flex items-center gap-2.5 mb-4">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${variationWarning ? 'bg-rose-50 border border-rose-200' : 'bg-emerald-50 border border-emerald-200'}`}>
                  <Gauge className={variationWarning ? 'text-rose-500' : 'text-emerald-600'} size={15} />
                </div>
                <div>
                  <h3 className="text-[12px] font-bold text-slate-800">Simulation Variation Gauge</h3>
                  <p className="text-[9px] text-slate-400 font-mono-data uppercase tracking-wider">UI Surrogate Spread · Not Confidence</p>
                </div>
              </div>
              <VariationGauge value={variationStd} />
              <div className="text-center mt-1">
                {variationWarning ? (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center justify-center gap-1.5 text-[10px] text-rose-500 font-semibold">
                    <AlertTriangle size={12} /> High demo variation — inspect inputs
                  </motion.div>
                ) : (
                  <div className="flex items-center justify-center gap-1.5 text-[10px] text-emerald-600 font-semibold">
                    <ShieldCheck size={12} /> Demo variation below display threshold
                  </div>
                )}
              </div>
            </motion.section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <KpiCard icon={Gauge} label="Seeded Variation σ" value={variationStd.toFixed(2)}
              sub={variationWarning ? 'Above UI display threshold' : 'Below UI display threshold'}
              danger={variationWarning} accent="emerald" />
            <KpiCard icon={ShieldCheck} label="Privacy Metric" value="Not measured" sub="No DP claim in this UI surrogate" accent="blue" />
            <KpiCard icon={Radio} label="Demo Partitions" value="3/3" sub="Synthetic telemetry · no hospitals connected" accent="emerald" />
          </div>
      ...
      <footer className="mt-8 flex flex-wrap items-center justify-between gap-3 text-[9px] text-slate-400 font-mono-data uppercase tracking-[0.15em] pb-4">
        <div className="flex items-center gap-2">
          ...
          <span>AegisOnco · Local Synthetic UI Telemetry</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="flex items-center gap-1"><TrendingUp size={10} className="text-emerald-500" /> Hand-Authored Curve</span>
          <span>·</span>
          <span className="flex items-center gap-1"><Brain size={10} className="text-violet-500" /> Notebook Artifacts Not Connected</span>
          <span>·</span>
          <span>Synthetic Data Only — Never Enter PHI</span>
        </div>
      </footer>
    </div>
  );
